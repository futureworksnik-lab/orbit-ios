...
## active: CP-4 complete — 10 core components (`OrbitButton`, `LoadingButton`, `OrbitTextField`, `OTPBoxField`, `OrbitCard`, `OpenerChip`, `VibeTag`, `Avatar`, `EmptyStateView`, `SkeletonView`) built entirely on CP-3 tokens, full snapshot-test coverage. Merged to `main` via PR #8 (squash `3a681d8`). Full story, decisions, and debt in `docs/checkpoints/CP-4-summary.md` — read that before touching `Components/` or `.github/workflows/ci.yml`. Highlights:

- `OpenerChip`/`VibeTag` are two separate types (not one mode-enum component); `LoadingButton` is a thin async wrapper around `OrbitButton` — both founder-confirmed decisions.
- `swift-snapshot-testing` 1.19.4 added, `OrbitTests`-only.
- **`.github/workflows/ci.yml`'s simulator selection is no longer dynamic** — pinned to iPhone 16/iOS 26.2, because pixel-snapshot tests (new in CP-4) don't survive CI's old "highest available OS" scan across a window-server-not-started hang, a display-scale mismatch, and cross-OS-point-release font rendering, all three hit and fixed on this PR. Local dev machines on a different iOS runtime (e.g. this repo's own 26.5) will see occasional harmless near-miss snapshot failures — CI's pinned simulator is authoritative, not local. If the pin's fallback warning ever fires (runner image dropped that combo), re-pin and re-record all 32 references, don't ignore it.
- Independently re-verified on `main` post-merge (not just trusted CI): build/lint/banned-words clean, 80/81 local tests (the one expected near-miss).
- Next: CP-5 (app shell, navigation, coordinator).
...
- CP-4 (Core Components) is complete and merged — see the `active:` line above and
  `docs/checkpoints/CP-4-summary.md` for the full per-component design, decisions, and the CI
  pipeline saga; not re-duplicated here.
